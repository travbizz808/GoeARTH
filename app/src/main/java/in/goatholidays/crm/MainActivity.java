...
public class MainActivity extends android.app.Activity {
    private static final String HOME_URL = "https://crm.goatholidays.in/login";
    ...
    private void clearWebCacheOncePerAppVersion() {
        ...
        SharedPreferences preferences = getSharedPreferences("goat_holidays_crm_app", MODE_PRIVATE);
    ...
    private void configureSystemBarsSafely() {
        ...
        int chromeColor = Color.rgb(5, 45, 82);
    ...
    private void configureWebView() {
        ...
        webView.addJavascriptInterface(new DownloadBridge(), "GoatDownload");
        ...
        webView.setDownloadListener((url, userAgent, contentDisposition, mimeType, contentLength) -> {
            ...
            try {
                ...
                request.setDescription("GOAT Holidays CRM");
    ...
    private void installGeneratedDownloadInterceptor(WebView view) {
        String script =
                "(function(){" +
                "if(window.__goatGeneratedDownloadFix)return;" +
                "window.__goatGeneratedDownloadFix=true;" +
                "function nameFor(a){return (a&&a.getAttribute&&a.getAttribute('download'))||('GOAT-Holidays-Itinerary-'+Date.now()+'.pdf');}" +
                "async function save(url,name){try{" +
                "var response=await fetch(url);var blob=await response.blob();var reader=new FileReader();" +
                "reader.onloadend=function(){GoatDownload.saveBase64File(reader.result,name,blob.type||'application/pdf');};" +
                "reader.onerror=function(){GoatDownload.reportError('Unable to read generated file');};reader.readAsDataURL(blob);" +
                "}catch(e){GoatDownload.reportError(String(e));}}" +
    ...
    private void downloadGeneratedFile(String url, String fileName, String mimeType) {
        ...
        String script =
                ...
                "reader.onloadend=function(){GoatDownload.saveBase64File(reader.result," +
                jsQuote(finalFileName) + ",blob.type||" + jsQuote(finalMimeType) + ");};" +
                "reader.onerror=function(){GoatDownload.reportError('Unable to read generated PDF');};" +
    ...
    private String normalizeDownloadFileName(String fileName, String mimeType) {
        ...
        if (name.isEmpty() || name.equalsIgnoreCase("download") || name.startsWith("blob")) {
            name = "GOAT-Holidays-Itinerary-" + System.currentTimeMillis();
        }
